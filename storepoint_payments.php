...
if($_GET['location'] != ''){
	...
	while($row3 = mysql_fetch_assoc($result3)){
        ...
        //if((float)$row3['unapplied_amt']<=0){ $class.= ' zero';
                                            
                $first_table .= "<tr >";
                $first_table .= "<td style=\"text-align:center;\">";
                $first_table .= "<a data-toggle=\"modal\" class=\"mymodal\"  client='".$_REQUEST["location"]."' rel='".$row3['id']."' href=\"mymodal\" style=\"text-decoration:underline;\">".$row3['id']."</a></td>
		...
		$submit_total += (float)$row3['unapplied_amt'];
	}
}
...
div.table-wrapper div.scrollable { overflow: scroll; overflow-y: hidden; }
    ...
    .btnblack{color: black !important;}.btnblack:link{color: black !important;}
    .btnblack:visited{color: black !important;}
    .btnblack:hover{color: black !important;}
    .btnblack:active{color: black !important;}body{top:0px!important;font-size:12px!important;}table{font-size:12px!important;}.goog-te-banner-frame{margin-top: -50px!important;}.sorting_asc{background: url('images/sort_asc.png') no-repeat center right !important;
    background-color: #333333 !important;}.sorting_desc{background: url('images/sort_desc.png') no-repeat center right !important;background-color: #333333 !important;}

      .mymodal2:hover{color:#0866C6;}.line3{background: #808080;}
      .line3{background-color:#808080; color:#000000 !important;}
  }
</style>
...
<div class="mainwrapper">
  ...
  <div class="rightpanel">
    <ul class="breadcrumbs">
      ...
      <li>VendorPanel <span class="separator"></span></li>
      <li>Payments</li>
    ...
    <div class="pageheader"> 
			...
			<input type="text" id="keyword" onKeyDown="javascript:if(event.keyCode==13){ return false; }" name="keyword" onBlur="search_ven(this.value)" placeholder="Search Locations" class="go_search2" style="width: 190px; padding-left: 5px; height: 33px; float:right; margin-right:10px;">
			</div>
			<?php } ?>
...
<table id="licence_table" class="table table-bordered responsive">
                            ...
                            <table id="licence_table" class="table table-bordered responsive">
                                        ...
                                        <?php
										$total=0;
										$result3 = mysql_query($query3) or die('err10'.mysql_error());
						...
						<div class="clearfix"> 
							...
							<table id="licence_table" class="table table-bordered responsive">
											...
											<tr class="<?=$unapplied;?>" id='<?=$row4['id'];?>'>
													...
													<? //$img;?>
													<?php if($submit_total > 0){ ?><span class="icon-pencil"></span><?php }else{ ?> <span class="icon-search"></span> <?php } ?></a></td>
											</tr>
											<?php } }?>
											</tbody>
						  </table>
				  ...
				  <tbody>
											...
											<tr class="<?=$unapplied;?>" id='<?=$row4['id'];?>'>
													...
													<a data-toggle="modal" class="mymodal3" unapplied_amount='<?=$total;?>' vendor_id='<?=$row4['location_id'];?>'   client='<?=$row4['id'];?>' rel='<?=$row4['id'];?>' href="mymodal3">
													<? //$img;?>
													<span class="icon-search"></span>                                               </a>												</td>
											</tr>
											<?php } } ?>
											</tbody>
						  </table>
...
<div id="mymodal" class="modal hide fade" style="width:760px;margin-left:-380px;">
	...
	<div class="modal-footer" style="text-align:center;">
		<a data-dismiss="modal" href="#" class="btn btnblack">Close</a>
		<button href="javascript:void(0);" onClick="printDiv('mymodal_html')" pri_location='' pri_rel='' class="btn btn-primary print_data">Print</button>
	</div>
</div>
...
<div id="mymodal3" class="modal hide fade" style="height: 500px;">
	<div class="modal-header">
		<button type="button" class="close" data-dismiss="modal"
			aria-hidden="true">&times;</button>
...
function printDiv(divName){
  ...
  console.log('storepoint_payments_prints.php?orderid='+orderid+'&client_id="'+client_id);
	var WindowObject = window.open('storepoint_payments_prints.php?orderid='+orderid+'&client_id="'+client_id, 'PrintWindow', 'width=750,height=650,top=0,left=50,toolbars=no,scrollbars=yes,status=no,resizable=yes');
  jQuery.ajax({
		url:"storepoint_payments_prints.php?orderid="+orderid+"&client_id="+client_id,
		success:function(result){
			jQuery("#loading-header").hide();	
			console.log(result);
			var WindowObject = window.open('storepoint_payments_prints', 'PrintWindow', 'width=750,height=650,top=0,left=50,toolbars=no,scrollbars=yes,status=no,resizable=yes');
...
/* function printDiv(divName) {
    ...
    jQuery(document).ready(function() {
        ...
        jQuery(".mymodal").click(function(){
			...
			jQuery('.print_data').attr("pre_rel",orderid);
			jQuery('.print_data').attr("pre_location",client_id);
