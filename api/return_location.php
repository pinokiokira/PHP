...
//                                            $imagefile=API."panels/teampanel/images/primary-type/Default Primary Type - ".$primarytypename.".png";
			...
			if ($location->address !="") $locationsAssociatedWithEmployee .= $location->address;
			if ($location->address !="" && $location->address2 !="") $locationsAssociatedWithEmployee .= ', ';
			if ($location->address2 !="" ) $locationsAssociatedWithEmployee .= $location->address2;
                                        
			$locationsAssociatedWithEmployee .= '<br>';
			if ($location->city !="") $locationsAssociatedWithEmployee .= $location->city;
			if ($location->city !="" && $state->name !="") $locationsAssociatedWithEmployee .= ', ';
			if ($state->name !="") $locationsAssociatedWithEmployee .= $state->name;
			if ($location->zip !="" && $state->name !="") $locationsAssociatedWithEmployee .= ', ';
			if ($location->zip !="") $locationsAssociatedWithEmployee .= $location->zip;
			if ($location->zip !="" && $country->name !="") $locationsAssociatedWithEmployee .= ', ';
			if ($country->name !="") $locationsAssociatedWithEmployee .= $country->name;
		...
		$sql="SELECT em.location_id, em.status, em.emp_id, em.id, em.first_name, em.last_name, loc.primary_type, loc.email FROM employees em
			INNER JOIN locations loc ON loc.id = em.location_id
			WHERE loc.email='".$employeeMaster->email."' AND loc.status='active' AND  em.status='A'";
		if($statusWhr=='Active'){
			//$sql .= " AND loc.status='active' AND em.status='A'"; 
		}
								
		$sql .= " GROUP BY em.location_id ORDER BY loc.name";
		$exe = mysql_query($sql);
		while($row = mysql_fetch_array($exe)){
			...
			$locationsLinkedWithEmployee .= '<div class="locBoox">';
			$locationsLinkedWithEmployee .= '<div class="img-box"><img style="float:left; margin-right:10px; margin-top:8px;width:50px;height:50px" src="'.$imagefile.'"></div>';
			...
			$locationsLinkedWithEmployee .= 'Email: '.$row["email"];
			$locationsLinkedWithEmployee .= '<br>';
			...
			if ($location->address !="") $locationsLinkedWithEmployee .= $location->address;
        	if ($location->address !="" && $location->address2 !="") $locationsLinkedWithEmployee .= ', ';
            if ($location->address2 !="" ) $locationsLinkedWithEmployee .= $location->address2;
                                        
			$locationsLinkedWithEmployee .= '<br>';
			if ($location->city !="") $locationsLinkedWithEmployee .= $location->city;
			if ($location->city !="" && $state->name !="") $locationsLinkedWithEmployee .= ', ';
			if ($state->name !="") $locationsLinkedWithEmployee .= $state->name;
			if ($location->zip !="" && $state->name !="") $locationsLinkedWithEmployee .= ', ';
			if ($location->zip !="") $locationsLinkedWithEmployee .= $location->zip;
			if ($location->zip !="" && $country->name !="")	$locationsLinkedWithEmployee .= ', ';
			if ($country->name !="") $locationsLinkedWithEmployee .= $country->name;
			$locationsLinkedWithEmployee .= '<br>';
