...
$limit = 10;
if(isset($_GET["records_show"])){
	$limit = mysql_real_escape_string($_GET["records_show"]);
}
...
if (($_GET['g'] != '' || $_GET['vendor']!='' || $_GET['serach_item']!='' ) && $_GET['item'] == '') { //echo "group";  //Group has been selected
	...
	$val_10 = ($limit == 10) ? 'selected' : '';
	$val_25 = ($limit == 25) ? 'selected' : '';
	$val_50 = ($limit == 50) ? 'selected' : '';
	$val_100 = ($limit == 100) ? 'selected' : '';
	
	$str .= '<label style="float:right">'.$_SESSION['first_name'] . ' ' . $_SESSION['last_name'].'</label>';
	$str .= '<label>Show <select id="records_show" data-g="'.$_GET['g'].'" data-item="'.$_GET['item'].'"  data-market="'.$_GET['market'].'" data-vendor="'.$_GET['vendor'].'" data-search_item="'.$_GET['search_item'].'" data-storeroom="'.$_GET['storeroom'].'">
				<option value="10" '.$val_10.'>10</option>
				<option value="25" '.$val_25.'>25</option>
				<option value="50" '.$val_50.'>50</option>
				<option value="100" '.$val_100.'>100</option></select></label>';
    ...
    while ($row8 = mysql_fetch_array($result8)) {
        if (isset($_GET['t'])) {
            $query7 = "SELECT lic.Type,lic.date_counted,lic.quantity,lic.time_counted,employees.first_name, employees.last_name,inventory_item_unittype.unit_type,lic.created_datetime,inventory_item_unittype.id as unit_id
                       ...
                       ORDER BY lic.date_counted DESC, lic.time_counted DESC LIMIT ".$limit."";
            ...
            $query7 = "SELECT Type,date_counted,time_counted,inv_item_id,lic.unit_type, quantity,e.first_name,e.last_name,iiu.unit_type as unit,lic.unit_type,lic.created_datetime,iiu.id as unit_id
                       ...
                       ORDER BY lic.id DESC LIMIT ".$limit."";
        }
        $queryy = mysql_query("SELECT SUM(lic.quantity) AS total_qty 
            FROM location_inventory_counts lic
            LEFT JOIN inventory_item_unittype ON inventory_item_unittype.id=lic.unit_type
            WHERE inv_item_id=" . mysql_real_escape_string($_GET['item']) . "");
        ...
        if ($num_rows > 0) {
            ...
            $str .= '<tr class="widgettitle">
                    ...
                    <tr>
                        ...
                        <td style="width:15% !important;">
                            <input type="text" rel="'.$j.'" id="counts" onKeyPress="return isNumberKey(event)"  style="width:80% !important;margin-bottom: 2px !important;" placeholder="'.$_SESSION['Count'].'" name="count[]" class="enable incnt"/>
                        </td>
                    </tr>';
            ...
            while ($row7 = mysql_fetch_array($result7)) {
                ...
                $qty = $row7['quantity'];
				$total[] = $row7['quantity'];
				$unit_with = $row7["unit_type"];
				$total_with[$unit_with][] = $row7['quantity'];
				$unit_with = ($row7["unit_type"] != '') ? $row7["unit_type"] : 'NO';
                if (!$stop) {
            ...
            $str .= "<tr><td colspan='6' style='text-align: right;'>Total: " . number_format(array_sum($total),2,'.','')."</td></tr>";
            /* if(!empty($row_7['quantity']))
            {
               
			    if($_REQUEST['debug']!=''){
					echo '<br> Before <br><pre>';
					print_r($inventoryItemCount);
					echo '<br> +'.$row_7['quantity'];
					echo '<br></pre>';
				}
				//$inventoryItemCount[$unit] = $inventoryItemCount[$unit]+$row_7['quantity'];
				if(array_sum($total) != '0'){
					$inventoryItemCount[$row8['stroom_id']][$unit] = number_format(array_sum($total),2,'.','');
				}
				//echo "<pre>"; print_r($inventoryItemCount); echo "<br>";
				if($_REQUEST['debug']!=''){
	...
	//echo count($total_with);
	//echo "<br>";
	//echo "<pre>";print_r($total_with);echo "<br>";
	foreach($total_with as $uType=>$quan){
		if($uType != ''){
			if(array_sum($quan) != '0'){
				$inventoryItemCount[][$uType] = number_format(array_sum($quan),2,'.','');
			}
			//echo "type = ".$uType." ---  Price = ". array_sum($quan);
		}
		//echo "<br>";
	}
    ...
    if($cnt>0)
        ...
        '<colgroup>
                ...
                <col style="" class="con0">
                    ...
                    <thead>
                        <tr>
                            <th class="head0 left">Storeroom Count</th>
