...
{
        $ch1= curl_init();
        $url = API ."panels/vendorpanel/api/generatetoken.php";
...
elseif($_GET['url']=='signup_process'){
	$url = API .'panels/vendorpanel/api/signup_process.php?first_name='.$_POST['first_name'].'&last_name='.$_POST['last_name'].'&email='.$_POST['email'].'&password='.$_POST['password'];
}
	...
	$_POST['email'] = $email;
        $url = DOMAIN . '/vendorpanel/ajax/forgot_password_process.php';
}
	...
	$url = API . 'panels/vendorpanel/api/upload_resume.php';
	...
	$url = API . 'panels/vendorpanel/api/upload_profile_photo.php';
	...
	$url = API . 'panels/vendorpanel/api/remove_profile_photo.php';
	$_POST['image'] = base64_encode(file_get_contents('../images/default_avatar.png'));
}
else if($_GET['url']=='check_availability'){
	$url = API . 'panels/vendorpanel/api/check_availability.php';
}
else if($_GET['url']=='return_client.php'){
	$url = API . 'panels/vendorpanel/api/return_client.php';
}
else if($_GET['url']=='return_vendor.php'){
	$url = API . 'panels/vendorpanel/api/return_vendor.php';
}
else if($_GET['url']=='return_location.php'){
	$url = API . 'panels/vendorpanel/api/return_location.php?token='.$token.'&client_id='.$_POST['client_id'];
}
else if($_GET['url']=='update_client.php'){
	$url = API . 'panels/vendorpanel/api/update_client.php';
}
else if($_GET['url']=='return_countryandtype.php'){
	$url = API . 'panels/vendorpanel/api/return_countryandtype.php';
}
else if($_GET['url']=='return_states.php'){
	$url = API . 'panels/vendorpanel/api/return_states.php';
}
else if($_GET['url']=='return_currency.php'){
	$url = API . 'panels/vendorpanel/api/return_currency.php';
}
else if($_GET['url']=='client.php'){
	$url = API . 'panels/vendorpanel/api/client.php';
}
else if($_GET['url']=='update_clientSM.php'){
	$url = API . 'panels/vendorpanel/api/update_clientSM.php?token='.$token;
}
else {
	$url = API . $_GET['url'];
}
