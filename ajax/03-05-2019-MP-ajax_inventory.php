...
if (($_GET['g'] != '' || $_GET['vendor']!='' || $_GET['serach_item']!='' ) && $_GET['item'] == '') { //echo "group";  //Group has been selected
    ...
    while ($row8 = mysql_fetch_array($result8)) {
        ...
        if ($num_rows > 0) {
            ...
            $str .= "<tr><td colspan='6' style='text-align: right;'>Total: " . number_format($roww['total_qty'],2,'.',''). "</td></tr>";
            if(!empty($row_7['quantity']))
            {
				...
				if(array_sum($total) != '0'){
					$inventoryItemCount[$row8['stroom_id']][$unit] = number_format($roww['total_qty'],2,'.','');
				}
