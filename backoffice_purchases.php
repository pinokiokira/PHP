...
//echo $hide;
//echo "<pre>"; print_r($items_by_group);
?>
<!DOCTYPE html>
<html>
<head>
...
<div class="mainwrapper">
    ...
    <div class="rightpanel">
        ...
        <div class="pageheader" id="date_search_box">
         ...
         <!--<a href="#" class="btn btn-success btn-large sameasorderd">Same as Ordered</a>-->
                 <?php if($status != "Completed"){?>
                <a href="#" class="" id="submit_order" title="Submit Order">
					<button  id="sub_btn" class="btn btn-success btn-large" <?php if($_REQUEST['reorder']!='Yes') { ?> disabled="disabled" <?php } ?> >Submit</button>
				</a>
                <?php }else{ ?>
                      ...
                      <div class="span12">
						...
						<form id="order_frm" method="post">
							<table class="topTbl table table-bordered table-infinite">
								<tr>
									<td style="width:15%;" >Location:<span style="color:#FF0000;">* </span></td>
									<td style="width:35%;">
										...
										<?php } else { ?>
											<select id="vendor" name="vendor" class="frmstyle1" style="width:99.8% !important">
                                                ...
                                                <option value="find"> - - - Find Location - - -</option>
							...
							<table  class="table table-bordered table-infinite" id="item_tbl">
								<tbody>
                                    ...
                                    <tr>
                                        ...
                                        <?php /*<input class="btn btn-success btn_table_top" type="button" id="addItem" value="Add New Item" <?=$disabled_1;?> style="<?=$hide;?>"> */?>
                                        <h4 class="widgettitle">Ordered Items / Shipped Items</h4></td>
                                    </tr>
                                    <tr>

												<th class="head1" >Group</th>
												<th class="head0" >Item Description&nbsp;<img src="images/add.png" id="add-item" alt="Add New Item" title="Add New Vendor Item" style="cursor:pointer;position:relative;bottom: 2px;height: 14px;" />
												</th>
									...
									<tr class="title">
										<td>Group</td>
										<td>Item Description &nbsp;
												<img src="images/add.png" id="add-item" alt="Add New Item" title="Add New Vendor Item" style="cursor:pointer;position:relative;bottom: 2px;height: 14px; <?php echo $hide; ?>"/>
										</td>
										<td class="itm_inpt_th">Pack Unit Type</td>
										<td class="itm_inpt_th">Qty in Pack</td>
										<td class="itm_sel_th">Qty in Pack Unit Type</td>
										<td class="itm_sel_th">Qty in Pack Size</td>
									...
									<?php if($_GET['purchase_id'] != ''){
										...
										while($row3 = mysql_fetch_assoc($result3)){
											...
											$q = mysql_fetch_array(mysql_query('SELECT lii.id AS loc_inv_id
												...
												WHERE vi.vendor_id = "'.$row3['vendor_id'].'" AND ii.inv_group_id="'.$row3['inv_group_id'].'" AND lii.inv_item_id="'.$row3['vi_inv_item_id'].'" AND lii.location_id = "'.$_SESSION['loc'].'"'));
											...
											<tr id="<?=$k?>" class="item_row oomt <?= $k . "_ord" ?>" data-inv_group_id="<?=$row3['inv_group_id']?>">
												<td>
													<select class='frmstyle2 group' <?=$disabled_1;?> style="width: 115px;" <?php echo ($status == "Ordered") ? 'disabled' : '' ?>>
												...
												<td>
													<select class='frmstyle2 item' name='item[]' <?=$disabled_1;?> style="width: 115px;" <?php echo ($status == "Ordered") ? 'disabled' : '' ?>>
												...
												<td style="text-align: center;">
                                                    <select <?php echo ($status == "Ordered") ? 'disabled' : '' ?> class="itm_sel pack_unittype" name="pack_unittype[]" <?=$disabled_1;?> style="width: 100px;">
												...
												<td>
                                                    <input <?php echo ($status == "Ordered") ? 'disabled' : '' ?> type='text' class="itm_inpt qty_in_pack" name="qty_in_pack[]" <?=$disabled_1;?> value="<?=number_format($row3['ordered_qty_in_pack'],2)?>" style="width: 50px;"/>

												</td>
												<td>
													<select <?php echo ($status == "Ordered") ? 'disabled' : '' ?> class="itm_sel qty_pack_unittype" name="qty_pack_unittype[]" <?=$disabled_1;?> style="width: 100px;">
														<option value="">Select Unit</option>
														<option value="new">Add New Unit</option>
														<?php while($row8 = mysql_fetch_assoc($result_unit2)){?>
															<option value="<?=$row8['unit_id']?>" <?php if($row8['unit_id'] == $row3['ordered_pack_unittype']){ echo "selected='selected'"; } ?>><?=$row8['unit_type'];?></option>
												...
												<td style="text-align: center;">
													<input <?php echo ($status == "Ordered") ? 'disabled' : '' ?> type='text' class="itm_inpt pack_size" name="pack_size[]" value="<?=number_format($row3['ordered_pack_size'])?>" <?=$disabled_1;?> style="width: 50px;" />
												</td>
												<td style="text-align: center;">
													<input disabled type='text' class="itm_inpt qty_on_hand" name="qty_on_hand[]" value="<?= $qty_on_hand['qty']; ?>" <?=$disabled_1;?> style="width: 50px;"/>
												</td>
												<td style="text-align: center;">
													<input <?php echo ($status == "Ordered") ? 'disabled' : '' ?> type='text' class="itm_inpt qty" name="qty[]" value="<?=number_format($row3['ordered_quantity'],2)?>" <?=$disabled_1;?> style="width: 50px;"/>
												</td>
												<td style="text-align: center;">
													<input <?php echo ($status == "Ordered") ? 'disabled' : '' ?> type='text' class="itm_inpt price" name="price[]" value="<?='$'.number_format($row3['ordered_price'],2)?>" <?=$disabled_1;?> readonly />
												</td>
												<td style="text-align: center;">
                                            ...
                                            <tr id="<?=$k?>" class="ramt <?= $k . "_ship" ?>" style="outline: 1px solid #0866c6;">
												...
												<td>
                                                <input disabled="disabled" type='text' class="itm_inpt r_qtypk" name="rec_qty_in_pack[]" value="<?=number_format($row3['ordered_qty_in_pack'],2)?>"/>
												</td>
												<td>
                                                ...
                                                <input  disabled="disabled" type='text' class="itm_inpt rec_pack_size" name="rec_pack_size[]" value="<?=number_format($row3['ordered_pack_size'])?>" />
												</td>
										<!-- 	</tr>
											<tr id="<?=$k."_rec"?>" class="ramt"> -->
												<td>
                                                <input disabled onKeyPress="return isNumberKey(event)" type='text' class="itm_inpt rec_qty_on_hand" name="rec_qty_on_hand[]" value="<?= $qty_on_hand['qty'] ?>" />
												</td>
                                            	<td>
                                                <input <?php echo ($status == "Shipped") ? 'disabled' : ''; ?> onKeyPress="return isNumberKey(event)" type='text' class="itm_inpt rec_qty" name="rec_qty[]" value="<?=$row3['shipped_quantity']?>" />
												</td>
												<td>
                                                <input  disabled="disabled" type='text' class="itm_inpt rec_price" name="rec_price[]" data-price_val = "<?=number_format($row3['ordered_price'],2)?>" value="<?='$'.number_format($row3['ordered_price'],2)?>" />
												</td>
												<td>
                                                <input disabled="disabled" type='text' class="itm_inpt rec_tax_percentage tax_<?=$k?>" name="rec_tax_percentage[]" value="<?=($row3['shipped_tax_percentage'] != "" ? $row3['shipped_tax_percentage'] : '$0.00')?>" />
												</td>
												<td>
                                                <input  type='text' class="itm_inpt r_amount amt_<?=$k?>" value="<?php echo '$'.number_format((($row3['shipped_price']+($row3['shipped_price']*$row3['shipped_tax_percentage']/100))*$row3['shipped_quantity']),2,'.',',') ?>" readonly name="r_amount" />
												</td>
												<td class="center">
...
function loadVendor(id,email,phone,name,image)
{
				...
				success:function(data){
					if(data.flag)
					{
						...
						jQuery.each(data.units, function (i, item) {
							...
							trtdelements  = '<tr>';
							trtdelements  += '<td style="padding:0px;" colspan="12">';
							...
							trtdelements  += '<img style="cursor:pointer;position:relative;bottom: 2px;height: 14px; " title="Add New Vendor Item" alt="Add New Item" id="add-item" src="images/add.png">';
							trtdelements  += '</td>';
							...
							trtdelements  += '<td class="itm_sel_th">Qty on Hand</td>';
							...
							trtdelements += '</td>';
							trtdelements += '<td style="text-align: center;">';
							trtdelements += '<input type="text" style="width: 50px;" value="1" name="qty_on_hand[]" class="itm_inpt qty_on_hand">';
							trtdelements += '</td>';
							trtdelements += '<td style="text-align: center;">';
							...
							clone_table_addeliment = clone_table_addeliment + '<td style="text-align:center; " class="last_td"><img style="" src="images/Delete.png" class="remove" /></td>';
	...
	jQuery('#comments').prop("readonly", false);
	...
	jQuery('#item_tbl tr').each(function () {
		...
		if (id != null && id != 'undefined') {
			if (jQuery('#' + id).hasClass('oomt')) {
				...
				//jQuery('.' + id + '_ord' ).find('.amt').val(jQuery('.' + id + '_ord' ).find('.price').val().replace('$',''));
				//jQuery('.' + id + '_ship' ).find('.r_amount').val(jQuery('.' + id + '_ship' ).find('.rec_price').val().replace('$',''));
