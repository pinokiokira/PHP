...
//	if (isset($_REQUEST['identifier']) && $_REQUEST['identifier'] == "employee") {
		...
		$sql = "SELECT  id,concat(first_name,' ',last_name,' ') name,emp_id FROM employees where location_id={$location} AND $nameq $numq order by name ASC ";
		if($_REQUEST['debug']!=''){
			echo '<br>'.$sql;	
		}
		...
		if ($r) {
			...
			while ($l = mysql_fetch_assoc($r)) {
				$result .= "<option value='".$l['id']."'>".$l['name']." (ID: ".$l['emp_id'].")</option>";
                                //$p = $l['name'];
				//$id = $l['id'];
