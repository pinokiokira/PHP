...
if($_REQUEST['debug']!=''){
	echo '<br>'.$_REQUEST['identifier'];
	echo '<br>'.$_REQUEST['query'];
	echo '<br>'. strlen($_REQUEST['query']);
}

if (isset($_REQUEST['query']) && $_REQUEST['query'] != ""&& strlen($_REQUEST['query'])>2) {
//juni 20.01.2013 -> change minimum size to 3 chars
	...
	if (isset($_REQUEST['identifier']) && $_REQUEST['identifier'] == "estado") {
	...
	}else if(isset($_REQUEST['identifier']) && $_REQUEST['identifier'] == "estadoAll"){
		...
		// $sql = "SELECT id,name FROM locations where $numq $nameq order by name ASC limit 10"; //juni -> filter on id also
		//mysql_set_charset('utf8',$con);
		$sql = "SELECT id,`name` FROM locations where 1 $nameq $numq order by name ASC limit 8";
		if($_REQUEST['debug']!=''){
			echo '<br>'.$sql.'<br>';
		}
		$res = mysql_query($sql) or die(mysql_error());
		if ($res) {
			while ($row = mysql_fetch_assoc($res)) {	
				...
				if (is_null($row['name'])||$row['name']==''){
					...
					$name = $row['name'].' (ID[[semicolon]]: '.$row['id'].')';
					$label =  htmlentities(strip_tags($name), ENT_QUOTES); 
				}
