            ...
            <div class="rightpanel">
                ...
                <div class="pageheader">
                    ...
                    <div style="float: right;margin-top: 10px;" class="messagehead">      
                    	<button onClick="getaddLocation(<?php echo $_SESSION["StorePointVendorID"]; ?>)" id="addcode" style="margin-left:10px;" class="btn btn-success btn-large">Add</button>
      				</div>      
                                ...
                                <table class="table table-bordered responsive" id="global_tbl">
                                    ...
                                    <tbody>
                                        ...
                                        while ($row = mysql_fetch_array($resultJobs)) {
                                            //print_r($row);die;
                                            $last_o_date = mysql_fetch_array(mysql_query("SELECT DATE_FORMAT(order_datetime,'%Y-%m-%d %H:%i') as order_datetime  from purchases as p JOIN employees_master as em ON em.StorePoint_vendor_Id = p.vendor_id WHERE p.location_id = '" . $row['location_id'] . "' and em.empmaster_id = '" . $_SESSION['empmaster_id'] . "' AND order_datetime is not null order by order_datetime desc limit 1"));
                                            $l_date = $last_o_date['order_datetime'] ? $last_o_date['order_datetime'] : '&nbsp;';

                                            /* $unreadmsggque = "select COUNT(a.message) AS unread_count1
                                              from employee_master_location_storepoint a,  locations
                                              where a.emp_master_id='".$_SESSION['empmaster_id']."' AND a.location_id = '".$row["location_id1"]."'
                                              and locations.id =  '".$row["location_id1"]."'
                                              and a.`read` = 'No' and a.sent_by_type='".$row["sent_by_type"]."'"; */
                                            $unreadmsggque = "select COUNT(a.message) AS unread_count1
												 from employee_master_location_storepoint a,  locations
												 where a.emp_master_id='" . $_SESSION['client_id'] . "' AND a.location_id = '" . $row["location_id1"] . "'
												  and locations.id =  '" . $row["location_id1"] . "'
												 and a.read = 'No' and a.sent_by_type='" . $row["sent_by_type"] . "'";

                                            $unreadmsggrs = mysql_query($unreadmsggque);
                                            $unreadmsgg = mysql_fetch_array($unreadmsggrs);
                                            ...
                                            $sql_last_msg = "SELECT DATE_FORMAT(sent_datetime,'%Y-%m-%d') as date FROM employee_master_location_storepoint WHERE location_id = '" . $row["location_id"] . "'
						  				   ORDER BY sent_datetime DESC";
                                            $res_last_msg = mysql_query($sql_last_msg);
                                            $result_lm = mysql_fetch_array($res_last_msg);

                                            $image = APIPHP . "images/" . $row["image"];
                                            if (!isImage($image)) {
                                                $image = APIPHP . "panels/clientpanel/images/" . getPrimaryTypeImage($row['primary_type']);
                                            }
                                            ?>
                                            <tr class="gradeX" id="<?php echo $_SESSION["StorePointVendorID"]; ?>" l_id="<?php echo $row["location_id1"]; ?>" style="cursor:pointer;">
                                                <td class="left cl_order" style="vertical-align: middle"> 
    <?php
    if (isImage($image)) {
        ?>
                                                        <img src="<?php echo $image; ?>" style="max-height:50px; width:50px;"/>

        <?php
    } else {
        ?>
                                                        <img src="images/noimage.png" style="max-height:50px; width:50px;" />
        <?php
    }
    ?>
                                                </td>
                                                <td class="cl_order"><?php echo "<strong>" . $row["name"] . "</strong>" . "<br />" . $row["city"] . ", " . $result_s["name"] . "<br />" . $result_c["name"]; ?></td>
                                                <td class="cl_order"><?php echo $row["lname"]; ?></td>
                                                <td class="cl_order center"><?php echo $result_lm["date"]; ?></td>
                                                <td class="cl_order center"><?php echo $l_date; ?></td>
                                                <td class="cl_order center"><?php 
												//print_r($_SESSION['StorePointVendorID']);
												
												$salesvar = mysql_fetch_array(mysql_query("SELECT sales_variance FROM vendor_locations WHERE vendor_id = '".$_SESSION['StorePointVendorID']."' AND location_id = '".$row["location_id1"]."' LIMIT 1"));
												echo $salesvar['sales_variance']; ?></td>
                                                <td class="cl_order center"> 
                                                <?php 
                                                $get_reminder = mysql_query("SELECT reminder FROM vendor_locations WHERE vendor_id = '".$_SESSION['StorePointVendorID']."'  AND location_id = '".$row["location_id1"]."' LIMIT 1");
                                                if(mysql_num_rows($get_reminder)>0){
                                                    $rem_row = mysql_fetch_assoc($get_reminder);
                                                    if($rem_row['reminder'] != '0000-00-00 00:00:00'){
														if($rem_row['reminder']<date('Y-m-d')){
															echo '<span style="color:red">';
														}else{
															echo '<span>';
														}
                                                        echo date('Y-m-d', strtotime($rem_row['reminder']));
														echo '</span>';                                                            
                                                    }
                                                ...
                                                <td class="center" style="vertical-align:middle;">
                                                    ...
                                                    <a class="locationEditPopup cl_order1" href="#composeModal" data-toggle="modal" itemid="<?php echo $row["id"].'##'.$row["location_id1"]; ?>"><img src="images/edit.png" title="Edit" /></a>

                                                    <a href="storepoint_messages_inquiry.php?storepointlocid=<?php echo $row["location_id1"]; ?>&pt=<?php echo $row["sent_by_type"]; ?>" style="color:#000000;">
                                                        <img src="images/envelope_24_16.png">&nbsp; (<?php echo $unreadmsgg["unread_count1"]; //$row["unread_count"];  ?>)
                                                    </a>
                                                </td>
