...
if ($_POST['purchaseSubmit'] == "1") {
	...
	$fet_loc = mysql_fetch_array(mysql_query("SELECT location_id FROM vendor_purchases WHERE vendor_id = '".$vendor_id."' AND vendor_invoice_num = '".$vendor_invoice_num."' LIMIT 10"));
	...
	if($msg == 'UP'){
	    header("Location: storepoint_items.php?msg=Purchase Updated Successfully!");
	}else{
		header("Location: storepoint_items.php?msg=Purchase Added Successfully!");
	}
}
...
if ($_REQUEST["sort"] != "") {
    ...
    $ordquery = " ORDER BY p.shopping_datetime DESC";
}
if($_REQUEST['date']!=""){
...
if($_REQUEST['text_s']!=""){
	$s_q = $_REQUEST['text_s'];
	$text_filter =" AND (p.vendor_purchases_id LIKE '".$s_q."%' OR v.name LIKE '".$s_q."%' OR emp.first_name LIKE '".$s_q."%' OR emp.last_name LIKE '".$s_q."%' OR p.status LIKE '".$s_q."%' OR v.phone LIKE '".$s_q."%' )";
}
//Select purchase orders for location
 $query1 = "SELECT p.vendor_purchases_id as id, DATE_FORMAT(p.shopping_datetime,'%m/%d/%Y %H:%i') as shopping_datetime,DATE_FORMAT(p.order_datetime,'%m/%d/%Y %H:%i') as order_datetime,  p.status,
			...
			,(select sum(ordered_quantity) from purchase_items pi where pi.purchase_id=p.vendor_purchases_id) as qty
			,COALESCE(TRUNCATE((select sum(ordered_quantity*ordered_price) from purchase_items pi where pi.purchase_id=p.vendor_purchases_id)+p.tax_total,2),0.00) as total
           ...
           //WHERE 1=1 AND $date_filter $text_filter AND (p.status='Ordered')" . $ordquery; // OR p.status='Shipped'
$result1 = mysql_query($query1) or die(mysql_error());

//Select purchase orders for location
 $query_shipped = "SELECT p.vendor_purchases_id as id, DATE_FORMAT(p.shopping_datetime,'%m/%d/%Y %H:%i') as shopping_datetime,DATE_FORMAT(p.order_datetime,'%m/%d/%Y %H:%i') as order_datetime,  p.status,
			v.name,v.phone,v.contact as representative,v.StorePoint_image as image,p.vendor_invoice_num,v.city,vs.name as v_state,v.email as v_email
			,concat(emp.first_name,' ',emp.last_name) as representative1
			,(select sum(ordered_quantity) from purchase_items pi where pi.purchase_id=p.vendor_purchases_id) as qty
			,COALESCE(TRUNCATE((select sum(ordered_quantity*ordered_price) from purchase_items pi where pi.purchase_id=p.vendor_purchases_id)+p.tax_total,2),0.00) as total
		   ...
		   LEFT JOIN employees emp on p.lastchange_employee_id=emp.id
		   LEFT JOIN states as vs ON vs.id = v.state";
           //WHERE 1=1 AND $date_filter $text_filter AND (p.status='Shipped')" . $ordquery; // OR p.status='Shipped'
$result_shipped = mysql_query($query_shipped) or die(mysql_error());

//->juni [req REQ_018] - 2014-09-24 - add tabs
$queryShopping = "SELECT p.vendor_purchases_id as id, DATE_FORMAT(p.shopping_datetime,'%m/%d/%Y %H:%i') as shopping_datetime, p.total, p.status,
			v.name,v.phone,v.contact as representative,v.StorePoint_image as image,p.vendor_invoice_num,v.city, vs.name as v_state,v.email as v_email
			,concat(emp.first_name,' ',emp.last_name) as representative1
			,(select sum(ordered_quantity) from purchase_items pi where pi.purchase_id=p.vendor_purchases_id) as qty
           FROM vendor_purchases p
		   ...
		   LEFT JOIN employees emp on p.lastchange_employee_id=emp.id
		   LEFT JOIN states as vs ON vs.id = v.state";
           //WHERE 1=1 AND $date_filter $text_filter AND p.status='Shopping'" . $ordquery;
...
$queryCompleted = "SELECT p.vendor_purchases_id as id, DATE_FORMAT(p.completed_datetime,'%m/%d/%Y %H:%i') as shopping_datetime, p.total, p.status,
			v.name,v.phone,v.contact as representative,v.StorePoint_image as image,p.vendor_invoice_num,v.city, vs.name as v_state,v.email as v_email
			,concat(emp.first_name,' ',emp.last_name) as representative1
			,(select sum(received_quantity) from purchase_items pi where pi.purchase_id=p.vendor_purchases_id AND pi.received = 'yes') as qty
           FROM vendor_purchases p
           LEFT JOIN vendors v ON v.id=p.vendor_id
		   LEFT JOIN employees emp on p.lastchange_employee_id=emp.id
		   LEFT JOIN states as vs ON vs.id = v.state";
           //WHERE 1=1 AND $date_filter $text_filter AND (p.status='Completed' OR p.status='Cancelled')" . $ordquery;
...
if (isset($_GET["flag"])) {
    if ($_GET["purchase_id"] != ""){//Viewing a previously submitted purchase
        ...
        $query2 = "SELECT p.vendor_id, shopping_datetime, lastchange_datetime, p.status,
                          p.terms, subtotal, tax_total, total, vendors.name vname, p.comments,p.delivery_method,p.payment_type,p.vendor_invoice_num
                    FROM vendor_purchases p
        ...
        $query3 = "SELECT vi.vendor_id,vi.inv_item_id as vi_inv_item_id,pi.inv_item_id,ii.inv_group_id,pi.ordered_quantity,pi.ordered_pack_size,pi.ordered_pack_unittype,pi.ordered_qty_in_pack,pi.ordered_qty_in_pack_unittype,pi.ordered_price,pi.ordered_tax_percentage,
...
if($code_id!=""){
	...
	header('location:backoffice_purchases_order.php?flag&purchase_id='.$purid.'&msg=Item Added/Updated Successfully!');
