...
if ($_POST['count_submit'] == 'submitted') {
	...
	$page = $_REQUEST['page'];
	header('location:backoffice_inventory.php?market='.$_POST['market'].'&group_id='.$_POST['group_id'].'&vendor='.$_POST['vendor'].'&item_id='.$item_id.'&page='.$page.'&t');
...
if(!isset($_GET['st']) || $_GET['st'] == ''){
	...
	if($_REQUEST['debug']!=''){
		echo 'api/backoffice/get-movements.php?market='.$market.'&group_id='.$group_id.'&location=' . $_SESSION['loc'] . '&storeroom=' . $st . '&website&keyword='.$keyword.'&token1=' . $token1 .'&token='.$token;
	}
...
var intRoomHeight = jQuery('.widgetcontent').height();
    function onclk(item) {
        window.location.href = 'backoffice_inventory.php?g=<?=$_GET['g'];?>&item=' + item
    }
	...
	/* jQuery(document).on('change','#storeroom_page',function(){
		
		var str_room_page = jQuery(this).val();
		var str_room_id = jQuery(this).data('storeroom_id');
		var g = jQuery(this).data('g');
		...
		var st = '<?php echo $_REQUEST['st'] ?>';
		
		console.log('str_room_page : '+str_room_page);
		console.log('str_room_id : '+str_room_id);
		console.log('g : '+g);
		console.log('item : '+item);
		
		
		jQuery('.item_row').trigger('click');

		//document.location="backoffice_inventory.php?serach_item="+serach_item+"&group_id="+group_id+"market="+market+"vendor="+vendor+"st="+st+"g="+g+"item="+item+"str_room_page="+str_room_page+"str_room_id="+str_room_id;
		
	}); 
	
	<tr>
			<td colspan="6">
				<select id="storeroom_page" data-storeroom_id="'.$row8['storeroom_id'].'" data-g="'.trim($_REQUEST['g']).'" data-item="'.$_REQUEST['item'].'"">
					<option>5</option>
					<option>10</option>
					<option>15</option>
				</select>
				<input type="text" name="search" value="'.$row8['storeroom_id'].'"/>
			</td>
		</tr>
	*/
</script>
        ...
        <div class="pageheader">
            ...
            <input id="f_val" class="hidden" value="">
                ...
                $qry_vendors = "SELECT distinct(id),name FROM vendors WHERE 
					...
					<select id="status" name="st" style="float:right; width:150px; height:43px;padding: 8px 8px;margin:12px 10px 0 0;">
                                ...
                                <div class="detaildiv">
                                            ...
                                            <form id="count_frm" method="post">
                                                ...
                                                <table class="table table-bordered table-infinite" id="itm_detail">
                                                    ...
                                                    <thead>
                                                        <tr>
                                                            <th class="head0 center">Storeroom</th>
...
<script type="text/javascript">
    jQuery(document).ready(function ($) {
		...
		var vendor = '<?php echo $_REQUEST['vendor']; ?>';
		var serach_item = '<?php echo $_REQUEST['serach_item']; ?>';
		var storeroom = '<?php echo $_REQUEST['st']; ?>';
		...
		$.ajax({
				...
				type: "GET",
				data: "g="+gid+"&vendor="+vendor+"&serach_item="+serach_item+"&market="+market+"&storeroom="+storeroom,
		...
		jQuery("#records_show").live('change',function(){
			
			var pagin = '';
			var val = jQuery(this).val();
			var gid = jQuery(this).data('g');
			var id = jQuery(this).data('item');
			var market = jQuery(this).data('market');
			var vendor = jQuery(this).data('vendor');
			var serach_item = jQuery(this).data('serach_item');
			var storeroom = jQuery(this).data('storeroom');
			
			var data = "g="+gid+"&item="+id+"&page="+pagin+"&market="+market+"&vendor="+vendor+"&serach_item="+serach_item+"&t"+"&storeroom="+storeroom+"&records_show="+val;
		...
		$('.item_row').live('click', function () { //alert('sad');
			...
			var serach_item = '<?php echo $_REQUEST['serach_item']; ?>';
			var storeroom = '<?php echo $_REQUEST['st']; ?>';
			console.log('storeroom2 :'+ storeroom);
			<?php if(isset($_GET['t'])){ ?>
			var data = "g="+gid+"&item="+id+"&page="+pagin+"&market="+market+"&vendor="+vendor+"&serach_item="+serach_item+"&t"+"&storeroom="+storeroom;
			<?php }else{ ?>
			var data = "g="+gid+"&item="+id+"&market="+market+"&vendor="+vendor+"&serach_item="+serach_item+"&page="+pagin+"&storeroom="+storeroom;;
			<?php } ?>
        ...
        $('#toggle_btn').click(function () {
			...
			var market = '<?php echo $_REQUEST['market']; ?>'; 
        	<?php if (isset($_GET['t'])) { ?>
            	window.location = "backoffice_inventory.php?group_id="+group_id+"&item_id="+item_id+"&market="+market;
            <?php } else { ?>
            	window.location = "backoffice_inventory.php?group_id="+group_id+"&item_id="+item_id+"&market="+market+"&t";
            <?php } ?>
			...
			var serach_item = '<?php echo $_REQUEST['serach_item']; ?>';
			var storeroom = '<?php echo $_REQUEST['st']; ?>';
			console.log('storeroom3 :'+ storeroom);
			if(f!=""){
				datastring = "g="+gid+"&item="+id+"&market="+market+"&vendor="+vendor+"&serach_item="+serach_item+"&t"+"&storeroom="+storeroom;
				$("#f_val").val("")
			}else{
				datastring = "g="+gid+"&item="+id+"&vendor="+vendor+"&serach_item="+serach_item+"&market="+market+"&storeroom"+storeroom;
				$("#f_val").val("t")
			}
