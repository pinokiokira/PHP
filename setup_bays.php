...
if(isset($_POST) && $_POST['Submit']=="Submit"){
    ...
    $vendor_bays_id = mysql_real_escape_string($_REQUEST['vendor_bays_id']);
    $bay_code = mysql_real_escape_string($_REQUEST['bay_code']);
    $bay_name = mysql_real_escape_string($_REQUEST['bay_name']);
    $bay_description = mysql_real_escape_string($_REQUEST['bay_description']);
    $bay_size = mysql_real_escape_string($_REQUEST['bay_size']);
 
	if(!empty($vendor_bays_id)){
		$last_by = $_SESSION['client_id'];
		$last_on = 'VendorPanel';
		$last_datetime = date('Y-m-d H:i:s');

		$sql = "UPDATE vendor_bays SET  status = '".$status."', bay_code = '".$bay_code."', bay_name = '".$bay_name."', bay_description = '". $bay_description ."', bay_size = '". $bay_size ."', last_by = '". $last_by."', last_on = '". $last_on ."', last_datetime = '".$last_datetime."' WHERE vendor_bays_id = '".$vendor_bays_id."'";
		$up_inv = mysql_query($sql); //or die(mysql_error());

	}else{ 
		$ins_query = "INSERT INTO vendor_bays SET
						status = '".$status."', 
					  bay_code = '".$bay_code."',
					  vendor_id = '". $vendor_id ."',
					  bay_name = '".$bay_name."',
		...
		if($res_ins){
			$vendor_bays_id = mysql_insert_id();//die;
		}
	}
	
    $res1 = mysql_query($query1);// or die(mysql_error());
    header('location:setup_bays.php?by=SoftPoint&vendor_bays_id='.$vendor_bays_id.'&msg=Item Added/Updated Successfully!');
}
 $limit = 500;
if(isset($_REQUEST['search_txt1'])){
	...
	if($_REQUEST['search_txt1']!=""){
		...
		$search_where = " AND (vb.bay_description LIKE '%".$search."%' OR vb.bay_name LIKE '%".$search."%' OR vb.bay_code LIKE '%".$search."%')";
	}
}

	$sql = "SELECT vb.status, vb.vendor_bays_id, vb.vendor_id, vb.bay_code, vb.bay_name, vb.bay_description, vb.bay_size, vb.created_on, vb.created_by, vb.created_datetime, vb.last_by, vb.last_on, vb.last_datetime FROM vendor_bays as vb
...
if(isset($_GET['vendor_bays_id'])){
    $selected=$_GET['vendor_bays_id'];
}else{
    $selected=0;
}
...
.line_bays{	background-color:#808080;color:#000000 !important; }
...
jQuery(document).ready(function($){
	...
	jQuery('#global_tbl_wrapper .dataTables_filter input').unbind('keypress keyup').bind('keypress keyup', function(e){
		console.log('Hello 111');
		jQuery('.line_bays').removeClass('line_bays');
		jQuery("#"+jQuery('#vendor_bays_id').val()).addClass("line_bays");
    ...
    jQuery('#addcode').click(function(){
        ...
        jQuery('#vendor_bays_id').val('');
        jQuery('#bay_code').val('');
        jQuery('#bay_name').val('');
        jQuery('#bay_description').val('');
        jQuery('#bay_size').val('');
    ...
    jQuery(".cl_order").live('click',function(){
           ...
           // console.log('market : '+jQuery(this).data('vendor_baysid'));
			jQuery('.line_bays').removeClass('line_bays');;
			jQuery('#'+jQuery(this).data('vendor_baysid')).addClass('line_bays');
			
			jQuery('#status').val(jQuery(this).data('status'));
            jQuery('#vendor_bays_id').val(jQuery(this).data('vendor_baysid'));
            jQuery('#bay_code').val(jQuery(this).data('bay_code'));
            jQuery('#bay_name').val(jQuery(this).data('bay_name'));
            jQuery('#bay_description').val(jQuery(this).data('bay_description'));
            jQuery('#bay_size').val(jQuery(this).data('bay_size'));
           ...
           // jQuery(".gradeX").attr("class","gradeX cl_order");
            //jQuery(this).attr("class","gradeX cl_order line_bays");
	...
	if (search_inpt!=null) {
            ...
            if (search_inpt.length > 0) {
                if(search_inpt.length < 3) {
                    ...
                    window.location="setup_bays.php?&search_txt1="+jQuery('#search_txt1').val();
                }
            }else{
...
<div class="mainwrapper">
  ...
  <div class="rightpanel">
    ...
    <div class="pageheader">
      ...
      <div class="pagetitle">
        <h5>List of the Bays and staging areas</h5>
        <h1>Setup - Bays</h1>
      </div>
    </div>
    <!--pageheader-->
          ...
          <div class="span8" style="width:70%;float:left; overflow:auto;">
            ...
            <table class="table table-bordered responsive" id="global_tbl">
              ...
              <thead>
                <tr>
                  <th class="head1">S</th>
                  <th class="head0">Bay Code</th>
                  <th class="head0">Bay Name</th>
                  <th class="head1">Bay Description</th>
                  <th class="head1">Bay Size</th>
              ...
              <tbody>
                <?php  while($row = mysql_fetch_array($resultJobs)){
                   ...
                   ?>
                <tr class="<?php if($selected==$row["vendor_bays_id"]){ echo "selected-row";} ?> gradeX cl_order " id="<?php echo $row["vendor_bays_id"];?>"
                        data-status ="<?php echo $row["status"];?>"
                        data-vendor_baysid ="<?php echo $row["vendor_bays_id"];?>"
                        data-bay_code ='<?php echo $row['bay_code']; ?>'
                        data-vendor_id = "<?php echo $row['vendor_id']; ?>"
                        data-bay_name = "<?php echo $row['bay_name']; ?>"
                        data-bay_description = "<?php echo $row['bay_description']; ?>"
                        data-bay_size="<?php echo $row['bay_size']; ?>"
                  ...
                  <td><?php echo $row['bay_code']; ?></td>
                  <td><?php echo $row['bay_name']; ?></td>
                  <td><?php echo $row['bay_description']; ?></td>
                  <td><?php echo $row['bay_size']; ?></td>
          ...
          <div class="span4">
			...
			<div class="widgetbox" id="fdetail" style="padding-left: 5%;">
				<form id="edit_from234" name="frm234" action="" method="post" class="edit_from" >
					...
					<p>
						<label>Bay Code:<span style="color:#FF0000;">*</span></label>
						<span class="field">
							<input type="text" class="input-xlarge" id="bay_code" value="" name="bay_code">
						</span>
					</p>
					<p>
						<label>Bay Name:<span style="color:#FF0000;">*</span></label>
						<span class="field">
							<input type="text" class="input-xlarge" id="bay_name" value="" name="bay_name">
						</span>
					</p>
					<p>
						<label>Bay Description:<span style="color:#FF0000;">*</span></label>
						<span class="field"></span>
						<span class="field" id="group_span">
							<textarea class="input-xlarge" id="bay_description" value="" name="bay_description" onBlur="getbarcode_description(1)" type="text" onKeyDown=" javascript:if(event.keyCode==13){getbarcode_description(2);  return false;}"></textarea>
						</span>
					</p>
					<p>
						<label>Bay Size:<span style="color:#FF0000;">*</span></label>
						<span class="field" id="drop_span">
							<input type="text" class="input-xlarge" id="bay_size" value="" name="bay_size">
						</span>
					</p>
					<p>
...
function getbarcode_description(val){
  var search_val = jQuery("#bay_description").val();

    if(search_val!=""){
        if( search_val.length>60){
            jConfirm('Please enter a max of 60 characters in Bay Description!', 'Alert Dialog', function(r) {
...
var search_val = jQuery("#barcode").val();
    ...
    if (!search_val.match(/^\d*(?:\.\d{1,2})?$/)) { //replace all spaces (just to be sure)
        jConfirm('Invalid Barcode Number!', 'Alert Dialog', function(r) {
        //jQuery('#barcode').focus();
        });
        jQuery('#popup_cancel').remove();
        return false;
    ...
    if( search_val.length<6){
        jConfirm('Please enter a minimum of 6 digit number only!', 'Alert Dialog', function(r) {
            //jQuery('#barcode').focus();
            });
            jQuery('#popup_cancel').remove();
            return false;
...
jQuery("#sub_form").click(function(e){
	...
	var search_val = jQuery("#bay_description").val();
	var bay_code_val = jQuery("#bay_code").val();
	
	if(jQuery('#status').val()==""){
        jAlert('Please Select status!','Alert Dialog');
        e.preventDefault();
    }else if(jQuery('#bay_code').val()==""){
        jAlert('Please Enter Bay Code!','Alert Dialog');
        e.preventDefault();
    }else if(bay_code_val.length > 8){
        jAlert('Please enter a max of 8 characters in Bay Code!','Alert Dialog');
        e.preventDefault();
    }else if(jQuery('#bay_name').val()==""){
        jAlert('Please Enter Bay Name!','Alert Dialog');
        e.preventDefault();
    }else if(jQuery('#bay_description').val()==""){
        jAlert('Please Enter Bay Description!','Alert Dialog');
        e.preventDefault();
    }else if(jQuery('#bay_size').val()==""){
        jAlert('Please Enter Bay Size!','Alert Dialog');
        e.preventDefault();
    }else if( search_val.length>60){
		jAlert('Please enter a max of 60 characters in Bay Description!','Alert Dialog');
        e.preventDefault();
    }else{
...
jQuery('#bay_description').blur(function() {
    var desc = jQuery('#bay_description').val();
    if( desc.length>=3){
        console.log('enter');
        /*jQuery.ajax({
            url:'setup_items.php',
