...
*  @version $Id: backoffice_reports_item_group.php , v 1.0 7:54 PM 9/3/2014 juni $
*  -> [REQ_014  - 03.09.2014]
...
$query1 = "SELECT tbl.*,iiu.unit_type FROM (
...
function savePDF()
{
	show_page = jQuery("#report_select1").val();       
 	...
 	group_field = jQuery("#group").val(); 
	document.location = 'http://'+pathURL+'?group='+group_field+'&download='+show_page;
}
...
function sendMail()
{
 	...
 	delayTimer = setTimeout(function() 
	{
		...
		var _group_field = jQuery("#group").val(); 
       ...
       //	var mailValid = mailValidate(_mailAddress);
    	...
    	//if(mailValid === true)
		//{
    		var url  = location.protocol +'//'+_pathURL+'?group='+_group_field+'&download='+_show_page;
			console.log(url);
			jQuery.ajax(
			{
				type: "GET",
...
jQuery(document).ready(function()
{
	...
	jQuery("#group").change(function()
	{
		...
		if(group_field != '')
		{
			document.location = pathurl+'?group='+group_field;
		}
	}); 
});
</script>
...
<body>
	<?php
		...
		$filename = ' - Items by Group Report - '. date('Y-m-d H:i:s',strtotime($date));
	?>
        ...
        <div class="pageheader">
            <form action="" method="post" class="searchbar" onSubmit="return false;">
                ...
                <select id="report_select1" class="buttonReport">
                    ...
                    <option value="item_group" selected>Items by Group</option>
        ...
        <div class="maincontent">
            <div class="maincontentinner">   
              	<div class="report-header">
              		<!--<span class="tabname"> <?=$_SESSION['user_full_name'];?> - <?=$_SESSION["user"];?></span> <div class="tabtitle">Items by Group</div><br/>-->
              	...
              	<div class="report-header-el" id="report-header-print">              		
					...
					-->					
					<div class="tabtitle">Items by Group</div>
					<br/>
                ...
                <div class="widget">
                        ...
                        <select id="orderfield" name="orderfield">
	                    	...
	                    	<option value="iiu.unit_type" <? if($_GET['orderfield']=='iiu.unit_type') echo "Selected"; else echo ""; ?>>Unit Type</option>
	                    	<option value="priority" <? if($_GET['orderfield']=='priority') echo "Selected"; else echo ""; ?>>Priority</option>
	                    </select>
						<?php
						//AND lii.location_id = '".$_SESSION['loc']."'
							$query2 = "SELECT DISTINCT(tbl.id),tbl.description FROM ( 
								(SELECT DISTINCT(ig.id) AS id,ig.description 
								FROM inventory_groups ig 
								INNER JOIN inventory_items ii ON ig.id=ii.inv_group_id 
								INNER JOIN location_inventory_items lii ON lii.inv_item_id=ii.id AND lii.type='global' WHERE ig.Market='Restaurant'
								ORDER BY ig.description ASC) 
								UNION ALL 
								(SELECT DISTINCT(ig.id) AS id,ig.description 
								FROM inventory_groups ig	
								INNER JOIN location_inventory_items lii ON lii.local_group_id=ig.id AND lii.type<>'global' WHERE ig.Market='Restaurant' 
								ORDER BY ig.description ASC)) AS tbl ORDER BY description LIMIT 500";
								
							//$query2 = "SELECT DISTINCT id,description FROM inventory_groups WHERE description<>'' ORDER BY description ASC";
							$result2 = mysql_query($query2) or die(mysql_error());
						?>
                        <select name="group" id="group">
                        	<option value='0' <?php if($_GET['group'] == '0') echo "Selected"; else echo ""; ?>>Select Group</option>
                            <?php
                            while($row2 = mysql_fetch_array($result2)){ ?>
                            <option value='<?=$row2['id'];?>' <?php if($_GET['group'] == $row2['id']) echo "Selected"; else echo ""; ?>><?=$row2['description'];?></option>
                            <?php } ?>
                        </select>
                    ...
                    <div class="widgetcontent">
										<?php/*<div class="report-header-el" id="report-header-print">              		
											<!--<span class="tablocation"> <? echo getLocationName($_SESSION["loc"]); ?> (ID#: <?=$_SESSION["loc"];?>)</span> <div class="tabtitle">Items by Group</div><br/>
											<span class="tabname"> <?=$_SESSION['user_full_name'];?> - <?=$_SESSION["user"];?></span><span class="tabdate"> <?=date("m/d/Y");?> - <?=date("H:i");?></span>
											-->
