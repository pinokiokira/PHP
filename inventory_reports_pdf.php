...
function GetLocationTimeFromServer($intLocationID, $servertime){
    ...
    $jsonurl = "https://" . "www.softpointdev.com/panels/businesspanel/" . $jsonurl;
...
class PDF extends FPDF 
{
	function Header() 
	{
		...
		if($_GET['download'] =='receivable_ageing'){//->juni [REQ_014] -> move page number to footer
			...
			if($_GET['download']  =='inv_vendor'){ //landscap pdf
			$this->SetMargins(0.5,0.5,0.5);
			$x_ = $this->GetX();
            $this->SetX($x_+1);
...
$filename = $location.' - Items Report - '. $arrDate['date'] .'.pdf';
...
else if($_GET['download'] =='inv_date')
{
	...
	$header = array('Date', 'Time', 'Group', 'Item', 'Storeroom', 'P', 'Type', 'UT','Qty');
	$header = array_map('strtoupper', $header);
...
else if($_GET['download'] =='inv_stroom')
{
	...
	if($_REQUEST['debug']=="1"){
		echo "D: ".$_GET['date']."<br>D : ".$date."<bR>".$sql;
	}
	$query = mysql_query($sql) or die(mysql_error());
...
else if($_GET['download'] =='order_group')
{
	...
	/*$sql = "SELECT tbl.item,tbl.priority,tbl.datetime,tbl.`group`,e.first_name,e.last_name,iiu.unit_type FROM (
...
//	$filename=  $location.'_orders_by_group_report_'.$arrDate['date'].'.pdf';
