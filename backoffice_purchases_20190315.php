...
/* $jsonurl = API."panels/BusinessPanel/api/getlocationtime.php?intLocationID=63246&server_time=".urlencode('12/19/2018 19:42');
...
echo "<pre>"; echo date('Y-m-d H:i',strtotime($ldatetitme));
die; */
...
function check_vendor_to_location_items($vendor_id,$location_id,$ven_item_id,$qty){
		...
		$ven_row = mysql_fetch_array(mysql_query("SELECT inv_item_id,price FROM vendor_items WHERE id = '".$ven_item_id."'"));
		...
		//$query3 = "INSERT INTO location_inventory_counts SET location_id = '".$location_id."', inv_item_id='".$new_item_id."', Type='Start', unit_type='".$inv_q['unit_type']."',quantity='".$qty."',date_counted='$date', time_counted='$time', employee_id='" . $_SESSION['employee_id'] . "', storeroom_id='$storeroom' ,created_by = '" . $_SESSION['employee_id'] . "',created_on ='BusinessPanel',created_datetime='".date('Y-m-d H:i:s')."'";
		//$result3 = mysql_query($query3) or die(mysql_error());
			...
			//$query3 = "INSERT INTO location_inventory_counts SET location_id = '".$location_id."', inv_item_id='".$new_item_id."', Type='Purchase', unit_type='".$inv_q['unit_type']."',quantity='".$qty."',date_counted='$date', time_counted='$time', employee_id='" . $_SESSION['employee_id'] . "', storeroom_id='$storeroom' ,created_by = '" . $_SESSION['employee_id'] . "',created_on ='BusinessPanel',created_datetime='".date('Y-m-d H:i:s')."'";
			//$result3 = mysql_query($query3) or die(mysql_error());
...
function unitCheck($var){
    if(!is_numeric($var)){//check if new unit and if so, add to unit types
...
//12/19/2018 19:42   63246
function GetLocationTimeFromServer($intLocationID, $servertime){
	/*$jsonurl = API."API2/getlocationtime.php?intLocationID=".$intLocationID."&server_time=".urlencode($servertime);*/
...
function checkTerms($terms,$vendor_id){
    ...
    if($terms!="") {//check if terms and if so, add to vendor terms
...
if ($_POST['purchaseSubmit'] == "1") {
    ...
    $taxtotal = mysql_real_escape_string($_POST['taxTotal']);
    ...
    if($purchase_id != ''){//Submitted previously created order
        ...
        $query = "UPDATE purchases SET " . $purchase_fields . $update_fields . " WHERE id=". $purchase_id;
        $result = mysql_query($query) or die(mysql_error());
        ...
        $query = "INSERT INTO purchases SET " . $purchase_fields . $ins_fields;
    ...
    for($i=0;$i<count($_POST['item']);$i++){
        ...
        $tax_percentage = mysql_real_escape_string($_POST['tax_percentage'][$i]);
		check_vendor_to_location_items($vendor_id,$_GET['loc_id'],$item,$qty);
        //Insert purchase items
        $query = "INSERT INTO purchase_items SET
                        ...
                        ordered_price='$price',
                        ordered_tax_percentage='$tax_percentage'";
        $result = mysql_query($query) or die(mysql_error());
	...
	if($msg == 'UP'){
	    header("Location: purchases.php?msg=Purchase Updated Successfully!");
	}else{
		header("Location: purchases.php?msg=Purchase Added Successfully!");
	}
}
...
//Variables to append to query on search
if ($_REQUEST["sort"] != "") {
    $ordquery = " ORDER BY " . $_REQUEST["sort"];
}else{
    $ordquery = " ORDER BY shopping_datetime DESC";
}
if($_REQUEST['date']!=""){
	$date_filter = "AND DATE_FORMAT(p.shopping_datetime,'%Y-%m-%d')='".$_REQUEST['date']."'";
}
if($_REQUEST['text_s']!=""){
	$s_q = $_REQUEST['text_s'];
	$text_filter ="AND (p.id LIKE '".$s_q."%' OR v.name LIKE '".$s_q."%' OR emp.first_name LIKE '".$s_q."%' OR emp.last_name LIKE '".$s_q."%' OR p.status LIKE '".$s_q."%' OR v.phone LIKE '".$s_q."%' OR p.po LIKE '".$s_q."%' )";
}
//Select purchase orders for location
 $query1 = "SELECT p.id, DATE_FORMAT(p.shopping_datetime,'%m/%d/%Y %H:%i') as shopping_datetime,DATE_FORMAT(p.order_datetime,'%m/%d/%Y %H:%i') as order_datetime,  p.status,p.po,
			v.name,v.phone,v.contact as representative,v.StorePoint_image as image,p.vendor_invoice_num,v.city,vs.name as v_state,v.email as v_email
			,concat(emp.first_name,' ',emp.last_name) as representative1
			,(select sum(ordered_quantity) from purchase_items pi where pi.purchase_id=p.id) as qty
			,COALESCE(TRUNCATE((select sum(ordered_quantity*ordered_price) from purchase_items pi where pi.purchase_id=p.id)+p.tax_total,2),0.00) as total
           FROM purchases p
           LEFT JOIN vendors v ON v.id=p.vendor_id
		   LEFT JOIN employees emp on p.lastchange_employee_id=emp.id
		   LEFT JOIN states as vs ON vs.id = v.state
           WHERE p.location_id='" . $_GET['loc_id'] . "' $date_filter $text_filter AND (p.status='Ordered')" . $ordquery; // OR p.status='Shipped'
$result1 = mysql_query($query1) or die(mysql_error());

//Select purchase orders for location
 $query_shipped = "SELECT p.id, DATE_FORMAT(p.shopping_datetime,'%m/%d/%Y %H:%i') as shopping_datetime,DATE_FORMAT(p.order_datetime,'%m/%d/%Y %H:%i') as order_datetime,  p.status,p.po,
			v.name,v.phone,v.contact as representative,v.StorePoint_image as image,p.vendor_invoice_num,v.city,vs.name as v_state,v.email as v_email
			,concat(emp.first_name,' ',emp.last_name) as representative1
			,(select sum(ordered_quantity) from purchase_items pi where pi.purchase_id=p.id) as qty
			,COALESCE(TRUNCATE((select sum(ordered_quantity*ordered_price) from purchase_items pi where pi.purchase_id=p.id)+p.tax_total,2),0.00) as total
		   ...
		   LEFT JOIN employees emp on p.lastchange_employee_id=emp.id
		   LEFT JOIN states as vs ON vs.id = v.state
           WHERE p.location_id='" . $_GET['loc_id'] . "' $date_filter $text_filter AND (p.status='Shipped')" . $ordquery; // OR p.status='Shipped'
$result_shipped = mysql_query($query_shipped) or die(mysql_error());

//->juni [req REQ_018] - 2014-09-24 - add tabs
$queryShopping = "SELECT p.id, DATE_FORMAT(p.shopping_datetime,'%m/%d/%Y %H:%i') as shopping_datetime, p.total, p.status,p.po,
			v.name,v.phone,v.contact as representative,v.StorePoint_image as image,p.vendor_invoice_num,v.city, vs.name as v_state,v.email as v_email
			,concat(emp.first_name,' ',emp.last_name) as representative1
			,(select sum(ordered_quantity) from purchase_items pi where pi.purchase_id=p.id) as qty
           FROM purchases p
		   ...
		   LEFT JOIN employees emp on p.lastchange_employee_id=emp.id
		   LEFT JOIN states as vs ON vs.id = v.state
           WHERE p.location_id='" . $_GET['loc_id'] . "' $date_filter $text_filter AND p.status='Shopping'" . $ordquery;
$resultShopping = mysql_query($queryShopping) or die(mysql_error());

if($_REQUEST['debug']=="1"){
	echo $queryShopping;
}

$queryCompleted = "SELECT p.id, DATE_FORMAT(p.completed_datetime,'%m/%d/%Y %H:%i') as shopping_datetime, p.total, p.status,p.po,
			v.name,v.phone,v.contact as representative,v.StorePoint_image as image,p.vendor_invoice_num,v.city, vs.name as v_state,v.email as v_email
			,concat(emp.first_name,' ',emp.last_name) as representative1
			,(select sum(received_quantity) from purchase_items pi where pi.purchase_id=p.id AND pi.received = 'yes') as qty
           FROM purchases p
           LEFT JOIN vendors v ON v.id=p.vendor_id
		   LEFT JOIN employees emp on p.lastchange_employee_id=emp.id
		   LEFT JOIN states as vs ON vs.id = v.state
           WHERE p.location_id='" . $_GET['loc_id'] . "' $date_filter $text_filter AND (p.status='Completed' OR p.status='Cancelled')" . $ordquery;
$resultCompleted = mysql_query($queryCompleted) or die(mysql_error());
//<-juni [req REQ_018] - 2014-09-24

if (isset($_GET["flag"])) {
    if ($_GET["purchase_id"] != ""){//Viewing a previously submitted purchase
        //Get details about purchase
        $purchase_id = mysql_real_escape_string($_GET["purchase_id"]);
        $query2 = "SELECT p.vendor_id, shopping_datetime, lastchange_datetime, p.status,
                          p.terms, subtotal, tax_total, total, po, vendors.name vname, p.comments,p.delivery_method,p.payment_type,p.vendor_invoice_num
                    FROM purchases p
                    INNER JOIN vendors ON vendor_id = vendors.id
                    LEFT JOIN vendors_terms_types vt ON vt.vendors_terms_types=p.terms
                    WHERE p.id='" . $purchase_id . "' AND location_id='" . $_GET['loc_id'] . "'";
		...
		$vendor_invoice_num = $row2['vendor_invoice_num'];
        $status = $row2['status'];
        ...
        //Get purchase items
        $query3 = "SELECT pi.inv_item_id,ii.inv_group_id,pi.ordered_quantity,pi.ordered_pack_size,pi.ordered_pack_unittype,pi.ordered_qty_in_pack,pi.ordered_qty_in_pack_unittype,pi.ordered_price,pi.ordered_tax_percentage,
                   ...
                   INNER JOIN vendor_items vi ON vi.id=pi.inv_item_id
                   INNER JOIN inventory_items ii ON ii.id=vi.inv_item_id
                   WHERE pi.purchase_id='" . $purchase_id . "' AND vi.vendor_id='" . $vendor_id . "'
        ...
        $query5 = "SELECT vi.id,ii.description as item, ii.inv_group_id
                   FROM vendor_items vi
                   INNER JOIN inventory_items ii ON ii.id=vi.inv_item_id
        ...
        if ($vendor_id != '') {
			...
			$query10 = mysql_fetch_array(mysql_query("SELECT IFNULL(MAX(CAST(vendor_invoice_num AS UNSIGNED))+1, 100000) as maxid FROM purchases WHERE vendor_id={$vendor_id}"));
        ...
        //Get vendors
        /*$query6 = "SELECT id,name
                       FROM vendors  where name <>''
                       ORDER BY name ASC";//WHERE id NOT IN(SELECT vendor_id as id FROM purchases WHERE status='Shopping' AND location_id = " . $_SESSION['loc'] . "  )*/
...
UNION
SELECT v.id,v.name from locations as l JOIN purchases as p ON p.location_id = l.id JOIN vendors as v ON v.id = p.vendor_id where l.id = '".$_GET['loc_id']."'
group by v.id";
        $result6 = mysql_query($query6) or die(mysql_error());
    }
}
//echo 'Test : '.$shopping_datetime;
...
//echo 'shopping_datetime 1: '.$shopping_datetime;
$shopping_datetime = ($shopping_datetime == '1969-12-31' || $shopping_datetime == '') ? date('Y-m-d h:i:s') : $shopping_datetime;
//echo "<br>";
//echo 'shopping_datetime 2: '.$shopping_datetime;

$qry_status = mysql_fetch_array(mysql_query("SELECT status FROM purchases WHERE id='".$_GET['purchase_id']."'"));
...
<!--JQuery UI-->
<!--<link rel="stylesheet" href="css/ui-lightness/ui.css" type="text/css" media="all">-->
<!--<link rel="stylesheet" href="css/ui-dialog-buttonset/ui-buttons.css" type="text/css" media="all">
...
if(jQuery('#inv_item1').val()!=""){
	...
	jQuery('#order_frm').live('change',function(){jQuery('#sub_btn').prop('disabled',false);})
function validate()
{
        ...
        <div class="pageheader" id="date_search_box">
            ...
            <div class="pull-right" style="margin-top: 14px;">
				<input type="hidden" id="hid_same_as_orderd" name="hid_same_as_orderd" value="no" />
				 <a href="purchases.php" class="btn btn-primary btn-large">Back</a>
				 <?php if($status == 'Ordered'){ ?><a href="#" class="btn btn-success btn-large same_as_orderd">Same as Ordered</a> <?php } ?>
				 <a href="#" class="btn btn-success btn-large new_purchase" id="new_purchase">Same as Ordered</a>
                ...
                <?php }else{ ?>
                <a href="backoffice_purchases.php?flag&purchase_id=<?=$_REQUEST['purchase_id']; ?>&reorder=Yes" class="" title="Reorder">
            ...
            <div class="maincontentinner">
                ...
                <?php if(!isset($_GET['flag'])){ //if(!isset($_GET['purchase_id'])){?>
								...
								<tbody>
									<?php
										...
										while ($row1 = mysql_fetch_array($result1)) {
											...
											?>
											<tr <?=$class;?> style="cursor:pointer;" onClick="window.location.href='backoffice_purchases.php?flag&purchase_id=<?=$row1["id"];?>'">
                                                ...
                                                <td><?= GetLocationTimeFromServer($_GET['loc_id'],$date); ?></td>
									...
									<?php
										...
										while ($row1 = mysql_fetch_array($resultShopping)) {
											...
											<tr <?=$class;?> style="cursor:pointer;" onClick="window.location.href='backoffice_purchases.php?flag&purchase_id=<?=$row1["id"];?>'">
												...
												<td><?=GetLocationTimeFromServer($_GET['loc_id'],$row1["shopping_datetime"]); ?></td>
                                                <td style="text-align:center;"><?=$row1['vendor_invoice_num'];?></td>
                                                <td style="text-align:center;"><?=$row1['po'];?></td>
												<td>
											  <?php
					...
					<!--<div class="span8">-->
                      <div class="span12">
						...
						<form id="order_frm" method="post">
									...
									<td><?php if($full_status == "Completed"){ ?>
										...
										<select id="status" name="status" class="frmstyle1" <?php // echo $disabled1;?> style="width:99.8% !important">
											...
											if($purchase_id != '' && $_GET['reorder']!='Yes'){?>
												...
												<option value="Cancelled" <?php if ($full_status == "Cancelled"){echo "selected='selected'";} ?>>Cancelled</option>
								...
								<tbody>
                                    ...
                                    <tr>
                                        <td colspan="11" style="padding:0px;">
                                          <input class="btn btn-success btn_table_top" type="button" id="addItem" value="Add New Item" <?=$disabled_1;?> style="<?=$hide;?>">
                                        <h4 class="widgettitle">Ordered Items / Received Items</h4></td>
                                    </tr>
                                    <tr>

												<th class="head1" >Group</th>
												<th class="head0" >Item &nbsp;<img src="images/add.png" id="add-item" alt="Add New Item" title="Add New Vendor Item" style="cursor:pointer;position:relative;bottom: -3px;height: 14px;" />
												</th>
												<th class="head1" >Pk Size / <span style="color:#0866C6;">Pk Size</span></th>
												<th class="head0" >Pk Unit/ <span style="color:#0866C6;">Pk Unit</th>
												<th class="head1" >Qty Pk / <span style="color:#0866C6;">Qty Pk</span> </th>
												<th class="head0" >Qty Pk Unit / <span style="color:#0866C6;">Qty Pk Unit</span></th>
												<th class="head1" >Qty / <span style="color:#0866C6;">Qty</span></th>
												<th class="head0" >Price / <span style="color:#0866C6;">Price</span></th>
												<th class="head1" >Tax % / <span style="color:#0866C6;">Tax %</span></th>
												<th class="head0" >Amount / <span style="color:#0866C6;">Amount</span></th>
												<th class="head0" ><span style="color:#0866C6;">Received</span></th>
											</tr>
                                    <?php }else{ ?>
                                    <tr>
                                        <td colspan="11" style="padding:0px;">
									...
									<tr class="title">
										<td>Group</td>
										<td>Item &nbsp;
												<img src="images/add.png" id="add-item" alt="Add New Item" title="Add New Vendor Item" style="cursor:pointer;position:relative;bottom: -3px;height: 14px; <?php echo $hide; ?>"/>
										</td>
										<td class="itm_inpt_th">Pack Size</td>
										<td class="itm_sel_th">Pack Unit</td>
										<td class="itm_inpt_th">Qty Pack</td>
										<td class="itm_sel_th">Qty Pack Unit</td>
									...
									<?php if($_GET['purchase_id'] != ''){
										...
										while($row3 = mysql_fetch_assoc($result3)){
											...
											<tr id="<?=$k?>" class="item_row">
												<td>
													<select class='frmstyle2 group' <?=$disabled_1;?> style="width: 115px;">
												...
												<td>
													<select class='frmstyle2 item' name='item[]' <?=$disabled_1;?> style="width: 115px;">
														<option value="">Select Item</option>
														<?php foreach($items_by_group[$row3['inv_group_id']] as $val){ ?>
															<option value="<?=$val['id']?>" <?php if($val['id'] == $row3['inv_item_id']){ echo "selected='selected'"; }?>><?=$val['item'];?></option>
														<?php }?>
													</select>
												</td>
												<td style="text-align: center;">
													<input type='text' class="itm_inpt pack_size" name="pack_size[]" value="<?=$row3['ordered_pack_size']?>" <?=$disabled_1;?> style="width: 50px;" />
												</td>
												<td>
													<select class="itm_sel pack_unittype" name="pack_unittype[]" <?=$disabled_1;?> style="width: 100px;">
														<option value="">Select Unit</option>
														<option value="new">Add New Unit</option>
														<?php while($row8 = mysql_fetch_assoc($result8)){?>
															<option value="<?=$row8['id']?>" <?php if($row8['id'] == $row3['ordered_pack_unittype']){ echo "selected='selected'"; } ?>><?=$row8['unit_type'];?></option>
														<?php }
														mysql_data_seek($result8,0)?>
													</select>
												</td>
												<td style="text-align: center;">
													<input type='text' class="itm_inpt qty_in_pack" name="qty_in_pack[]" <?=$disabled_1;?> value="<?=$row3['ordered_qty_in_pack']?>" style="width: 50px;"/>
												</td>
												<td>
													<select class="itm_sel qty_pack_unittype" name="qty_pack_unittype[]" <?=$disabled_1;?> style="width: 100px;">
														<option value="">Select Unit</option>
														<option value="new">Add New Unit</option>
														<?php while($row8 = mysql_fetch_assoc($result8)){?>
															<option value="<?=$row8['id']?>" <?php if($row8['id'] == $row3['ordered_qty_in_pack_unittype']){ echo "selected='selected'"; } ?>><?=$row8['unit_type'];?></option>
														<?php }
														mysql_data_seek($result8,0)?>
													</select>
												</td>
												<td style="text-align: center;">
													<input type='text' class="itm_inpt qty" name="qty[]" value="<?=$row3['ordered_quantity']?>" <?=$disabled_1;?> style="width: 50px;"/>
												</td>
												<td style="text-align: center;">
													<input type='text' class="itm_inpt price" name="price[]" value="<?=$row3['ordered_price']?>" <?=$disabled_1;?> readonly />
												</td>
												<td style="text-align: center;">
													<input type='text' class="itm_inpt tax_percentage" name="tax_percentage[]" value="<?=$row3['ordered_tax_percentage']?>" <?=$disabled_1;?> readonly />
												</td>
												<td style="text-align: center;">
													<input type='text' class="itm_inpt amt" value="" readonly <?=$disabled_1;?>  />
												</td>
												<td style="text-align:center;">
                                            ...
                                            <tr id="<?=$k."_rec"?>" class="ramt" style="outline: 1px solid #0866c6;">
												...
												<td>&nbsp;</td>
												<td>
                                                <input  disabled="disabled" type='text' class="itm_inpt rec_pack_size" name="rec_pack_size[]" value="<?=$row3['received_pack_size']?>" />
												</td>
												<td >
												...
												<td>
                                                <input disabled="disabled" type='text' class="itm_inpt r_qtypk" name="rec_qty_in_pack[]" value="<?=$row3['received_qty_in_pack']?>"/>
												</td>
												<td>
							...
							?>
								...
								<td>
                                <input type="text" id="sub_total_vari1" class="itm_inpt" style="width:100%;" value="<?=add_breket($ordr_subtotal-$ord_subtotal)?>" readonly <?=$disabled;?>>
								</td>
								</tr>
								<tr>
								<td>Tax:</td>
								...
								<td>
                                <input type="text" id="tax_vari1" class="itm_inpt" style="width:100%;" value="<?=add_breket($ordr_tax_total-$ord_tax_total)?>" readonly <?=$disabled;?>>
								</td>
								</tr>
								<tr>
								<td>Grand Total:</td>
								...
								<td>
                                <input type="text" id="total_vari1" class="itm_inpt" style="width:100%;" value="<?=add_breket(($ordr_subtotal-$ord_subtotal)+($ordr_tax_total-$ord_tax_total))?>" readonly <?=$disabled;?>>
								</td>
								</tr>
								</tbody>
                                </table>
                      ...
                      <div class="cart_sub">
                        <span>Subtotal:</span>
  											<input type="text" name="subTotal" id="subTotal" class="itm_inpt" value="<?=$subtotal?>" readonly <?=$disabled;?>>
                      </div>
										</td>
									</tr>
                      ...
                      <div class="cart_sub">
                        <span>Tax:</span>
  											<input type="text" name="taxTotal" id="taxTotal" class="itm_inpt" style="margin-right:5px" value="<?=$taxtotal?>" readonly <?=$disabled;?>>
                    </div>
										</td>
									</tr>
									<tr class="active">
                      ...
                      <div class="cart_sub">
                        <span>Total:</span>
  											<input type="text" name="total" id="total" class="itm_inpt" style="margin-right:5px" value="<?=$total?>" readonly <?=$disabled;?>>
                      </div>
										</td>
									</tr>
								</table>
    ...
    jQuery(document).ready(function ($) {
       ...
       jQuery('.new_purchase').click(function(){
            <?php if(isset($_GET['flag'])){ ?>
                ...
                if(r){
                    document.location = 'backoffice_purchases.php?flag';
                }
				});
            <?php }else{ ?>
                document.location = 'backoffice_purchases.php?flag';
	...
	$('.btn_save_item').live('click', function(){
        ...
        jConfirm("Changing order to Ordered or Cancelled will no longer allow you to modify. Continue?","Confirm Dialog",function(r){
            if(r){
                ...
                //$('.remove,#addItem').unbind('click').css('opacity',.3);
      ...
      if(jQuery('#status').val()!='Ordered'){
        ...
        $('.group').live('change', function(){
            ...
            $.ajax({
                ...
                //data: { group: $(this).val(), vendor: '<?=$vendor_id?>' },
				 data: { group: $(this).val(), vendor: vendorID },
                dataType: 'json',
                success: function(data){
                    ...
                    $(data).each(function(a,b){
                        select.append('<option value="' +  b.id + '">' + b.description + '</option>');
                    });
                    dfr.resolve();
                }
            });
        ...
        $('.item').live('change', function(){
            ...
            $.ajax({
                ...
                data: { item: $(this).val(), vendor: $("#vendor_id").val() },
                dataType: 'json',
                success: function(data){
                    data = data[0];
                    ...
                    items[rowid][2] = data.pack_unittype;//pack_unit_type
                    items[rowid][3] = data.qty_in_pack;//quantity in pack
                    items[rowid][4] = data.qty_in_pack_unittype;//quantity in pack unittype
        ...
        function recalcItemTotal(rowid){
            ...
            $('#' + rowid + ' td input.amt').val((items[rowid][10]).toFixed(2));
            recalcTotals();
        }
